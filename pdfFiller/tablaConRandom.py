...
obtengo una serie de items que se resume en las siguientes listas para
...
for key, values in celdasParaLlenar.items():
 print (key,values)
 if key in bex1 and values!=None: #Estar seguro que la celda contiene algún número
 ...
 elif key in bex2 and values!=None:
 ...
 elif key in bex3 and values!=None:
  fol = round(random.uniform(0.07,0.18),2)
  writer.update_page_form_field_values(writer.pages[0], {key: fol})   #Procedo a llenar con números aleatorios entre 0.07 y 0.18

# write "output" to PyPDF2-output.pdf
with open("conRandom.pdf", "wb") as output_stream:
